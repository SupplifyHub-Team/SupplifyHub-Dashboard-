import { ADDITIONAL_ADVS_TABLE } from "@/lib/constants";
import { patchAdditionalAdvs } from "@/services/AdvsServices";

import { useMutation, useQueryClient } from "@tanstack/react-query";
import { toast } from "sonner";
export default function usePatchAdditionalAdvs() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({
      requestId,
      operationType,
    }: {
      requestId: string;
      operationType: "Cancel" | "Accept";
    }) => patchAdditionalAdvs(requestId, operationType),

    onSuccess: (_data, variables) => {
      const actionText =
        variables.operationType === "Accept" ? "قبول" : "إلغاء";
      toast.success(` تم ${actionText} الطلب بنجاح`);
      queryClient.invalidateQueries({ queryKey: [ADDITIONAL_ADVS_TABLE] });
    },

    onError: (error: Error) => {
      console.error("Error accept advertisement:", error.message);
      toast.error("حدث خطاء حاول مرة اخرى");
    },
  });
}
